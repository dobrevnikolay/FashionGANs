...
for epoch in range(num_epochs):
    ...
    for i , data in enumerate(train_loader, 0):
        ...
        error_generator_score = loss(output, true_label)+10*torch.sum(loss_seg(mS_tilde,x_true_mS0),dim=0)
        error_generator_score.backward()
        # error_generator_seg = 10*torch.sum(loss_seg(mS_tilde,x_true_mS0),dim=0)
        # error_generator_seg.backward()
...
fig = plt.figure()
ax = fig.add_subplot(121)
ax.imshow(S_tilde_sample[2])

ax= fig.add_subplot(122)
ax.imshow(S_0_sample[2])
plt.show()
